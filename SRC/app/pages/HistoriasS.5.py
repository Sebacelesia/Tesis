...
- **EA (Enfermedad Actual):** describí evolución y síntomas referidos o evidentes, en orden temporal. No incluir hallazgos del examen físico. 
  Si mencionás fiebre, tos, exantema u otros síntomas, no los repitas literalmente en EF.
  Podés comenzar con fórmulas típicas del registro clínico como “Consulta en emergencia por…” o “Ingresa por…”.
- **EF (Examen Físico):** describí sólo lo constatado al examen físico. 
  Podés confirmar fenómenos mencionados en EA, pero de forma resumida y objetiva (por ejemplo, si en EA se dice “fiebre alta”, en EF basta con “febril, Tax 38.5°C”).
  Incluir siempre estado general, constantes vitales y hallazgos físicos relevantes (piel, mucosas, CV, PP, Abd, MMII, etc.).
  Podés usar abreviaciones o cierres habituales del registro hospitalario (por ejemplo, “SP”, “VEA”, “s/p foco evidente”).
  Evitá repetir frases textuales del EA, pero mantené coherencia clínica.
  
Reglas de estilo:
- Telegráfico, impersonal, sin conectores narrativos ni explicaciones.
- Usar abreviaciones locales: SF, AP, AQ, AEA, EA, EF, EG, CV, PP, MAV, RR, Tax, PBC, IRAB, HTA, DM2, TARV, SP, VEA, etc.
- Puede presentar ligeras variaciones de orden o puntuación como en registros reales (por ejemplo: “Apirética. Buen EG. Lúcida.”).
- No incluir diagnósticos, laboratorio ni plan de tratamiento.
- Estructura esperada: SF / AP / AQ / AEA / EA / EF.
- Cada sección debe tener contenido clínico; el EF siempre debe cerrar la nota con hallazgos desarrollados.
"""
...
Tu tarea es redactar **una única historia clínica sintética**, destinada exclusivamente a fines académicos.  
Debe ser **verosímil y médicamente coherente**, respetando el estilo, la estructura y la jerga del registro clínico rioplatense, según los ejemplos del few-shot.

---

### 🔒 Restricciones clínicas
1. Solo se permiten casos **de Infectología**.  
   Si la patología ingresada o seleccionada **no pertenece a la especialidad de Infectología**, devolvé exactamente:  
   > "❌ Error: el modelo está diseñado solo para historias clínicas de Infectología."
2. El caso debe ser **coherente**, sin contradicciones ni repeticiones innecesarias.  
   Ejemplo: si menciona fiebre en EA, no repetirla en antecedentes.
3. Si el cuadro incluye una patología o motivo incompatible (por ejemplo, “fractura”, “migraña”, “hipotiroidismo”), devolvé el mismo mensaje de error.
...
- Si algún campo no fue provisto, el modelo puede definirlo libremente dentro de la práctica infectológica.
- Si el modo es “libre”, el modelo debe generar edad, sexo, patología y motivo dentro del ámbito de Infectología, asegurando coherencia clínica y realismo.

---

### 🩺 Instrucción final
Generá **una única historia clínica sintética**, en formato texto plano,  
siguiendo todas las reglas del `system_prompt` y los ejemplos del few-shot.  
Debe sonar **como una historia clínica real**, escrita por un infectólogo de hospital público uruguayo.  
El texto debe cerrar siempre en EF, con hallazgos desarrollados, sin títulos ni explicaciones adicionales.
"""
