...
temperatura = st.radio(
    ...
    [
        ...
        "3️⃣ Guardia caótica (temperatura 0.9)"
    ],
    index=1  # Marca por defecto la opción intermedia (0.7)
)

# Traducir selección a valor numérico
...
La estructura es libre, y en la mayoría de los casos **no debe seguir un formato rígido**.  
Podés usar encabezados (AP, EA, EF) si aparecen naturalmente, pero la mayoría de los textos deben escribirse **en forma continua o con encabezados incompletos o mezclados**.  
Algunos casos deben comenzar directamente con el motivo o la evolución (“Ingresa por...”, “Consulta por...”).
No mantengas el mismo patrón entre casos.  
Podés cortar frases sin punto final o repetir abreviaciones de forma irregular.  
Evitá que el texto parezca una plantilla; debe sonar como una nota escrita apurada, con cortes y transiciones bruscas.
...
El estilo de redacción debe imitar de forma prioritaria los ejemplos del few-shot provistos.  
Reproducí su tono telegráfico, formato irregular, puntuación incompleta y variación de encabezados.  
No busques claridad ni completitud: priorizá parecer redactado por un médico humano en guardia, con apuro y leve desprolijidad.  
Si el few-shot incluye frases inconclusas, abreviaciones inconsistentes o repeticiones, imitá ese patrón.  
El few-shot define la forma final, no solo el tono general.

En aproximadamente un tercio de los casos, el texto puede ser más desordenado:  
sin todos los encabezados, con frases truncas, repeticiones o espaciado irregular.

El texto debe parecer escrito con apuro en guardia, no como un informe estructurado.  
Si hay dudas, preferí frases cortadas, encabezados omitidos o mezclados, y abreviaciones compactas (“RR90”, “Sat91AA”, “PA110/70”).
...
# ---------------------------------------------------------------------
            ...
            response =  model.generate_content ( prompt_total,generation_config={"temperature": temp_value})
